...
| --- | --- | --- |
| `reproducible_plot` | Equivalent data figure can be generated from committed processed data and repository scripts | `(111)` AHC, `(103)` AHC, rank-cumulative AHC, single-rank AHC, strain AHC, multipole coefficients, minimal-model scans |
...
| `workflow_required` | Compact data extraction still requires generated large Hamiltonian, HDF5, Wannier, or AHC intermediate files; the final PDF is not tracked until a repository plotting path exists | band convergence |
