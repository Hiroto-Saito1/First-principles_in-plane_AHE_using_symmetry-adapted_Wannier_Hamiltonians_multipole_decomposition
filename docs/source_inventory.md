...
| Archived workflow manifest | `REPO/data/source/workflow_manifests/multipole_coefficients/` preserves the recovered source chain, including the `Fe.py` -> `Fe_samb.py` / `Fe_matrix.pkl` -> `Fe_matrix.hdf5` -> `trs_py_ed_tb.hdf5` route, curated `Fe_all_35` generation and HDF5-shape log excerpts, the selected `z_i` labels from `out_trs_ed`, and a supporting archived `Fe_all_20_supporting_samb.py.gz` file that still contains those selected labels. |
