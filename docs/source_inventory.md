...
| Input data | `z_coefficients.dat` (not directly listed; expected next to script) and `energy_diff_35_*.out` (in `theta0_qe-7.2/`) |
...
| Destination | `REPO/data/source/multipole_coefficients/z_coefficients.dat` + (existing) processed CSV under `REPO/data/processed/multipole_coefficients/`; bar PDFs generated by existing `REPO/scripts/reproduce_figures/plot_multipole_coefficients.py`. Replace PDF-vector fallback. |
