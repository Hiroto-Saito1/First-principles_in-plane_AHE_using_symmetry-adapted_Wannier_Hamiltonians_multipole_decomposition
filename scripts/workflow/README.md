...
- `export_minimal_model_source.py`: condenses archived minimal-model
  ...
  plotting script.
- `export_fit_ahc_source.py`: condenses either an archived `angle_dep_ahc.xml`
  fit source or a committed compact AHC CSV branch into the compact CSVs used
  by the paper-facing `(111)` / `(103)` `fit_ahc` panels.
