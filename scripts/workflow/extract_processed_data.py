#!/usr/bin/env python3
"""Extract compact manuscript figure data from paper workflow outputs.
...
def extract_primary_ahc(source_root: Path, output: Path) -> None:
    sources = {
        "111": source_root / "tests/Fe/FM_sqa_111",
        "103": source_root / "tests/Fe/FM_sqa_103",
    }
    for plane, source in sources.items():
        ...
        write_json(
            target / "metadata.json",
            {
                ...
                "large_file_policy": "No raw DFT, Wannier, or AHC intermediate files are stored in Git.",
            },
        )


def extract_rank_resolved(source_root: Path, output: Path) -> None:
    base = (
        source_root
...
def extract_strain(source_root: Path, output: Path) -> None:
    source = source_root / "tests/Fe/FM_sqa_103_strained_along_103"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Root directory containing the small XML summaries produced by the paper workflows.",
    )
    parser.add_argument("--output", type=Path, default=Path("data/processed"))
    args = parser.parse_args()

    extract_primary_ahc(args.source_root, args.output)
    extract_rank_resolved(args.source_root, args.output)
    extract_strain(args.source_root, args.output)
