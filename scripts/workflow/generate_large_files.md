...
3. Decompose the desired Hamiltonian branch into that basis. For the
   manuscript bar plots, the archived workflow reads a decomposition HDF5 like
   `trs_py_ed_tb.hdf5`:
...
4. Export a compact coefficient CSV from the decomposition HDF5 plus
   `Fe_samb.py`:
...
5. Store the compact CSV at
   `data/processed/multipole_coefficients/multipole_coefficients.csv`.
6. Run `scripts/reproduce_figures/plot_multipole_coefficients.py`.
