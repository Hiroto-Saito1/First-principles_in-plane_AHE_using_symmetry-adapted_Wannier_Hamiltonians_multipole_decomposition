...
class Amn:
        ...
        for ik, k in enumerate(self.sym.irr_kpoints):
            ...
            for isym, s in enumerate(self.sym.s):
                ...
                if self.sym.t_rev[isym] == 1:
                    sk = -sk
                kdiff = k - sk
                if not np.allclose(kdiff, np.round(kdiff)):
                    continue
                ...
                amn_ik = np.einsum(
                    "ml,ln,n->mn", amn[ik, :, :], Rmat[isym, :, :], phase, optimize=True
                )
                if self.sym.t_rev[isym] == 1:
                    amn_ik = np.conj(amn_ik)
                amn_ik = np.einsum(
                    "ml,ln->mn", self.sym.repmat[ik, isym, :, :], amn_ik, optimize=True
                )
                amn_sym[ik, :, :] += amn_ik
            amn_sym[ik, :, :] /= nh
    ...
    def Umat_symmetrize(self, Umat, Umat_opt=None):
        ...
        if Umat_opt is not None:
            ...
            Umat_full_irk = np.einsum(
                "klm,kmn->kln", Umat_opt_irk, Umat_irk, optimize=True
            )
            Umat_full_irk = self.symmetrize_Gk(Umat_full_irk)
            Umat_full = self.symmetrize_expand(Umat_full_irk)
            Umat_new = np.einsum(
                "kml,kmn->kln", np.conj(Umat_opt), Umat_full, optimize=True
            )
        else:
            Umat_irk = self.symmetrize_Gk(Umat_irk)
            Umat_new = self.symmetrize_expand(Umat_irk)
        self.log.info(
            "symmetrize expand diff = %.5e", np.sum(np.abs(Umat_new - Umat)) / self.nk
        )
        return Umat_new
    ...
    def write_amn(self, file_amn):
        ...
        with open(file_amn, "w") as fp:
            ...
            for ik, m, n in itertools.product(
                range(self.nk), range(self.num_wann), range(self.num_bands)
            ):
                # num_bands, num_wann, nk
                fp.write(
                    "{0} {1} {2}  {3.real:18.12f}  {3.imag:18.12f}\n".format(
                        n + 1, m + 1, ik + 1, self.amn[ik, n, m]
                    )
                )
    ...
    def Umat(self, index_win=None, check=True, Umat_opt=None):
        ...
        if Umat_opt is None:  # Umat simply from Amn
            ...
            for k in range(self.nk):
                ...
                if index_win is not None:
                    ...
                    Umat[k, index_win[k, :], :] = np.matmul(
                        u[index_win[k, :], : self.num_wann], v
                    )
                else:
                    u, s, v = scipy.linalg.svd(m)
        ...
        if check and self.sym is None:
            ...
            for k in range(self.nk):
                assert np.allclose(
                    np.matmul(np.transpose(np.conj(Umat[k, :, :])), Umat[k, :, :]),
                    np.eye(self.num_wann),
                    atol=1e-5,
                ), "Umat is not unitary for k = {}".format(k)
    ...
    def _init_projection_centers(self):
        ...
        if (
            self.sym is not None
            and hasattr(self.sym, "centers")
            and self.sym.centers is not None
            and self.sym.centers.shape[0] == self.num_wann
        ):
        ...
        if (
            self.nnkp is not None
            and hasattr(self.nnkp, "nw2r")
            and self.nnkp.nw2r is not None
            and self.nnkp.nw2r.shape[0] == self.num_wann
        ):
    ...
    def projection_sym_mat(self):
        ...
        assert np.all(
            np.abs(Rshift - np.round(Rshift)) < 1e-3
        ), "Rshift is not a lattice vector"
