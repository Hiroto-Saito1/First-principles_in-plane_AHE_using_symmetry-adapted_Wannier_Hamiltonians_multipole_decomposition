...
class Nnkp:
            ...
            with open(file_nnkp) as fp:
                ...
                for i, line in enumerate(lines):
                    ...
                    if "begin kpoints" in line:
                        ...
                        self.kpoints = np.genfromtxt(
                            lines[i + 2 : i + 2 + self.nk], dtype=float
                        )

                    if "begin nnkpts" in line:
                        self.nb = int(lines[i + 1])
                        dat = np.genfromtxt(
                            lines[i + 2 : i + 2 + self.nk * self.nb], dtype=int
                        )
                        self.nnkpts = dat.reshape(self.nk, self.nb, 5)

                    if (
                        "begin projections" in line
                        or "begin spinor_projections" in line
                    ):
                        ...
                        atom_orb_uniq = sorted(
                            set(atom_orb_strlist), key=atom_orb_strlist.index
                        )
                        atom_pos_uniq = sorted(
                            set(atom_pos_strlist), key=atom_pos_strlist.index
                        )
                        self.atom_orb = []
                        for orb_str in atom_orb_uniq:
                            indexes = [
                                j
                                for j, x in enumerate(atom_orb_strlist)
                                if x == orb_str
                            ]
                        ...
                        for pos_str in atom_pos_uniq:
                            indexes = [
                                j for j, x in enumerate(atom_orb_uniq) if pos_str in x
                            ]
                            self.atom_pos.append(indexes)
                            self.atom_pos_r.append(
                                [float(x) for x in pos_str.split()[0:3]]
                            )
    ...
    def calc_bvec_list(self):
        ...
        for i in range(self.nb):
            bmat[i, :] = [
                self.bvec[i, a] * self.bvec[i, b]
                for a, b in itertools.product(range(3), range(3))
            ]
        delta_ab = np.array(
            [a == b for a, b in itertools.product(range(3), range(3))]
        ).astype(int)
