...
class Sym:
        ...
        with open(file_sym) as fp:
            ...
            for i in range(nblocks):
                ...
                for j in range(nlines):
                    d = fp.readline().split()
                    repmat[ik - 1, isym - 1, int(d[0]) - 1, int(d[1]) - 1] = float(
                        d[2]
                    ) + 1j * float(d[3])
            #
            ...
            for isym in range(nsym):
                ...
                for i in range(nlines):
                    d = fp.readline().split()
                    rotmat[isym, int(d[0]) - 1, int(d[1]) - 1] = float(
                        d[2]
                    ) + 1j * float(d[3])
                ...
                for isym, s in enumerate(self.s):
                    ...
                    if np.allclose(kdiff, np.round(kdiff)):
                        if equiv[ik] < 0:
                            ...
                            raise Exception(
                                "k corresponds to two different irr. kpoints"
                            )
        if np.any(equiv < 0):
    ...
    def search_symop(self, sym_list):
        ...
        for isym in range(self.nsym):
            ...
            if (
                np.allclose(s0, self.s[isym])
                and np.allclose(tdiff, np.round(tdiff), atol=1e-6)
                and t_rev == self.t_rev[isym]
            ):
                if self.spinors:
                    if t_rev == 1:
            ...
            for n in range(self.nbnd):
                ...
                if np.abs(vall - val) < 1e-8 and (not np.round(val) == 0):
                    if np.abs(np.round(val) - val) > 0.1 and n < self.nbnd - 1:
                        print(
                            ik, isym, n, self.repmat[ik, isym, n, n], val, np.round(val)
                        )
        ...
        if debug_print:
            ...
            for ik, isym in itertools.product(range(self.nks), range(self.nsym)):
                for m, n in itertools.product(range(self.nbnd), range(self.nbnd)):
                    ...
                    print(
                        "{:5d} {:5d} {:5d} {:5d} {:12.6f} {:12.6f}".format(
                            ik, isym, m, n, val.real, val.imag
                        )
                    )
