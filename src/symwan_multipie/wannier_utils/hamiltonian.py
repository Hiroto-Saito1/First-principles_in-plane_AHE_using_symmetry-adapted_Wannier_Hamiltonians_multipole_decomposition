...
_BOLTZMANN_OVER_EV = 8.617333262145e-5
...
class HamR:
    ...
    def __init__(
        ...
        *,
        ...
        """
        if irvec is not None or ndegen is not None or hrs is not None:
            if irvec is None or ndegen is None or hrs is None:
                raise ValueError("irvec, ndegen, and hrs must be provided together.")
            self.irvec = np.asarray(irvec, dtype=np.int64)
            self.ndegen = np.asarray(ndegen, dtype=np.int64)
            self.hrs = np.asarray(hrs, dtype=np.complex128)
            self.a = None if a is None else np.asarray(a, dtype=np.float64)
            self.Amnrs = None if Amnrs is None else np.asarray(Amnrs, dtype=np.complex128)
            self.nrpts = int(self.hrs.shape[0])
            self.num_wann = int(self.hrs.shape[1])
            self.ir0 = self._find_origin_index()
            ...
            raise ValueError("Neither array data nor tb_dat/hr_dat is given.")

    @classmethod
    def from_tb_dat(cls, path: Union[str, Path], is_reorder: bool = False) -> "HamR":
        return cls(tb_dat=Path(path), is_reorder=is_reorder)

    @classmethod
    def from_hr_dat(cls, path: Union[str, Path], is_reorder: bool = False) -> "HamR":
        return cls(hr_dat=Path(path), is_reorder=is_reorder)

    def _find_origin_index(self) -> int:
        if not hasattr(self, "irvec"):
            return -1
        ...
        try:
            ...
            for i in range(self.nrpts // 15 + 1):
                ...
                if len(ndegen) >= self.nrpts:
                    break
            self.ndegen = np.array(ndegen[: self.nrpts], dtype=np.int64)
            ...
            for i in range(self.nrpts):
                ...
                if np.all(self.irvec[i] == 0):
                    self.ir0 = i
                for _ in range(self.num_wann * self.num_wann):
                    row, col, tr, ti = fp.readline().split()
                    row_i = int(row) - 1
                    col_i = int(col) - 1
                    if is_reorder:
                        row_i = row_i // 2 + (self.num_wann // 2) * (row_i % 2)
                        col_i = col_i // 2 + (self.num_wann // 2) * (col_i % 2)
                    self.hrs[i, row_i, col_i] = float(tr) + float(ti) * 1j
    ...
    def _read_hr(self, hr_dat: Path, is_reorder: bool):
        ...
        try:
            ...
            for i in range(self.nrpts):
                for _ in range(self.num_wann * self.num_wann):
                    irx, iry, irz, row, col, tr, ti = fp.readline().split()
                    row_i = int(row) - 1
                    col_i = int(col) - 1
    ...
    def export_hr_dat(self, path: Union[str, Path]):
        self.export(path)

    def export_tb_dat(self, path: Union[str, Path]):
        if not hasattr(self, "a") or self.a is None:
            raise ValueError("tb.dat export requires lattice vectors `a`.")
        if not hasattr(self, "Amnrs") or self.Amnrs is None:
            raise ValueError("tb.dat export requires position matrices `Amnrs`.")
        ...
        with open(path, mode="w") as fp:
            ...
            for ir in range(self.nrpts):
                fp.write("\n")
                fp.write(
                    f"{self.irvec[ir, 0]} {self.irvec[ir, 1]} {self.irvec[ir, 2]}\n"
                )
