...
from .mag_sym import MagneticSpacegroupAnalyzer
...
class WannierSystem:
    ...
    def set_sym(self, magmoms: Optional[Union[str, np.ndarray]] = None):
        ...
        if magmoms is None:
            self.sym_ops = SpacegroupAnalyzer(
                self.win.structure
        ...
        if magmoms == "x" or magmoms == "y" or magmoms == "z":
            ...
            magmoms = [
                np.array(magmom[magmoms], dtype=np.float64)
            ] * self.win.structure.num_sites
    ...
    def _calc_irvec(self, rd: np.ndarray) -> Tuple[np.ndarray, int, np.ndarray]:
        ...
        # check sum rule
        assert (
            np.sum(1 / ndegen) - np.product(mp_grid) < 1e-8
